...
def calc_repetition_statistics(statistics, load_experiment_data_func,  *args, statistics_directory=None,
    ...
    if statistics_directory is None: statistics_directory = exputils.stat.DEFAULT_STATISTICS_DIRECTORY
...
def load_data_from_repetitions(repetition_directories, data_subdirectory=None):
    ...
    if data_subdirectory is None: data_subdirectory = exputils.logging.DEFAULT_LOG_DIRECTORY
...
def collect_counters(name, data):
    ...
    for rep_id, rep_data in data.items():
        ...
        elif is_same_counter and not np.array_equal(counters, rep_data[name]):
